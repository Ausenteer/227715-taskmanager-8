...
const getFilterItem = (name, number, isChecked = false, isDisabled = false) => {
  return `<input
    type="radio"
    id="filter__${name}"
    ...
    ${isChecked ? ` checked` : ``}
    ${isDisabled ? ` disabled` : ``}
  /><label for="filter__${name}" class="filter__label">${name}<span class="filter__all-count">${number}</span></label>`;
};

const getBoardItem = (color, text, isHashtag = false, isDate = false, isImage = false) => {
...
const getNewArrBoards = (min, max, arr) => {
  ...
  for (let i = 1; i < rand; ++i) {
    newArrBoards.push(getRandomElement(arr));
  }
  return newArrBoards;
};
...
const handleClickFilter = (element, i) => {
  element[i].addEventListener(`click`, () => {
    ...
    renderBoards(getNewArrBoards(2, 40, boards));
  });
};

const onClickFilter = () => {
  const filtersArrSelector = document.querySelectorAll(`.filter__label`);
